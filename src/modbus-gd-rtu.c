...
static ssize_t _modbus_rtu_send(modbus_t *ctx, const uint8_t *req, int req_length)
{
		...
		struct rt_serial_device *serial = ctx_rtu->dev;
	
		ret = rt_device_write(&serial->parent, 0, req, req_length);
		return ret;
}
...
static ssize_t _modbus_rtu_recv(modbus_t *ctx, uint8_t *rsp, int rsp_length)
{
		...
		if(RT_EOK == rt_sem_take(&rx_sem, 100)) {
			if(!serial->parent.read(&(serial->parent), 0, rsp, 1)) {
				return 0;
			}
			return 1;
		} else {
                                              ...
                                              int rsp_length)
{
    ...
    if (req[0] != rsp[0] && req[0] != MODBUS_BROADCAST_ADDRESS) {
        if (ctx->debug) {
            LOG_E("The responding slave %d isn't the requested slave %d\n",
                    rsp[0],
                    req[0]);
        }
        errno = EMBBADSLAVE;
...
static int _modbus_rtu_check_integrity(modbus_t *ctx, uint8_t *msg, const int msg_length)
{
    ...
    if (crc_calculated != crc_received) {
        if (ctx->debug) {
            LOG_E("ERROR CRC received 0x%0X != CRC calculated 0x%0X\n",
                    crc_received,
                    crc_calculated);
        }
...
/* POSIX */static int _modbus_rtu_connect(modbus_t *ctx)
{
		modbus_rtu_t *ctx_rtu = ctx->backend_data;
		struct rt_serial_device *serial = ctx_rtu->dev;
	
    /* set serial configure parameter */
    serial->config.baud_rate = ctx_rtu->baud;
...
static int _modbus_rtu_flush(modbus_t *ctx)
{
    ...
    modbus_rtu_t *ctx_rtu = ctx->backend_data;
		...
		int cnt = 0;
		while(serial->parent.read(&(serial->parent), 0, &c, 1) == 1)
			cnt++;
...
modbus_t *
modbus_new_gd_rtu(const char *device, int baud, char parity, int data_bit, int stop_bit)
{
    modbus_t *ctx;
    ...
    ctx_rtu->confirmation_to_ignore = FALSE;
		ctx_rtu->dev = (struct rt_serial_device*)serial;
